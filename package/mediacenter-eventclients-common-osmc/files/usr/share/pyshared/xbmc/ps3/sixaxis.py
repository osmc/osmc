...
keymap_sixaxis_keys = list(keymap_sixaxis.keys())
...
class sixaxis:
    ...
    def __init__(self, xbmc, control_sock, interrupt_sock):
        ...
        # HID Report:1
        _bytes = [0x52, 0x1]
        _bytes.extend([0x00, 0x00, 0x00])
        _bytes.extend([0xFF, 0x72])
        _bytes.extend([0x00, 0x00, 0x00, 0x00])
        _bytes.extend([0x02])  # 0x02 LED1, 0x04 LED2 ... 0x10 LED4
        ...
        # 0x80 = 1Hz
        # 0x40 = 2Hz
        _bytes.extend([0xFF, 0x00, 0x01, 0x00, 0x01])  # LED4 [0xff, 0xff, 0x10, 0x10, 0x10]
        _bytes.extend([0xFF, 0x00, 0x01, 0x00, 0x01])  # LED3 [0xff, 0x40, 0x08, 0x10, 0x10]
        _bytes.extend([0xFF, 0x00, 0x01, 0x00, 0x01])  # LED2 [0xff, 0x00, 0x10, 0x30, 0x30]
        _bytes.extend([0xFF, 0x00, 0x01, 0x00, 0x01])  # LED1 [0xff, 0x00, 0x10, 0x40, 0x10]
        _bytes.extend([0x00, 0x00, 0x00, 0x00, 0x00])
        _bytes.extend([0x00, 0x00, 0x00, 0x00, 0x00])

        control_sock.send(struct.pack("42B", *_bytes))
        data = control_sock.recv(1)
