...
pull_source "https://github.com/osmc/libdvdread/archive/17d99db97e7b8f23077b342369d3c22a6250affd.tar.gz" "$(pwd)/src"
if [ $? != 0 ]; then echo -e "Error downloading" && exit 1; fi
# Build in native environment
build_in_env "${1}" $(pwd) "libdvdread-osmc"
build_return=$?
if [ $build_return == 99 ]
then
	echo -e "Building libdvdread"
	out=$(pwd)/files
	...
	sed '/Depends/d' -i files-dev/DEBIAN/control
        ...
        echo "Depends: ${1}-libdvdread-osmc (=${VERSION_NUM})" >> files-dev/DEBIAN/control
	update_sources
	handle_dep "autoconf"
	handle_dep "automake"
	handle_dep "libtool"
	...
	dpkg_build files ${1}-libdvdread-osmc.deb
	dpkg_build files-dev ${1}-libdvdread-dev-osmc.deb
	build_return=$?
fi
teardown_env "${1}"
exit $build_return
