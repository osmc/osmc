...
SETS = {
    "uname": {
        ...
        "flags": ["-I", "--systeminfo"],
        "logs": [
            {"name": "UNAME", "key": "0wwkXuO5", "ltyp": "cl_log", "actn": "uname -a"},
            {
                "name": "cmdline",
                "key": "0wwYYuO5",
                "ltyp": "file_log",
    ...
    "mem": {
        ...
        "flags": ["-m", "--memory"],
        "logs": [
            {"name": "Memory", "key": "eWTP1Mc8", "ltyp": "cl_log", "actn": "free -m"}
        ],
    },
    "diskspace": {
        "order": 17,
        ...
        "flags": ["-D", "--disk"],
        "logs": [
            {"name": "Diskspace", "key": "qZy25Yas", "ltyp": "cl_log", "actn": "df -h"}
        ],
    },
    "boot": {
        "order": 18,
...
class CommandLine(object):
    ...
    def readlines(self):
        ...
        while "|" in self.command_list:
            ...
            chunk, self.command_list = (
                self.command_list[:idx],
                self.command_list[idx + 1 :],
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Uploads vital logs to %s. If the network is unavailable, logs are copied to the SD Card."
        % UPLOAD_LOC
    )
    ...
    parser.add_argument(
        "-A", "--all", action="store_true", dest="all", help="Include all logs"
    )
    parser.add_argument(
        "-T",
        action="store",
    ...
    for a in arguments:
        parser.add_argument(
            *a["flags"], action=a["action"], dest=a["dest"], help=a["help"]
        )
...
class Main(object):
    def __init__(self, copy, termprint):
        ...
        self.log_blotter = (
            []
        )  # list to hold all the lines that need to be printed/uploaded
        ...
        self.number_of_actions = sum(
            1 for k, v in SETS.items() if v.get("active", False)
        )
    ...
    def dispatch_logs(self):
        ...
        if self.copy_to_boot:
            ...
            if CALLER == "kodi":
                ...
                log(
                    "Logs copied to /boot/%s on the SD card FAT partition"
                    % os.path.basename(TEMP_LOG_FILE)
                )
            ...
            attempts = ["curl -X POST -s    -T", "curl -X POST -s -0 -T"]
            ...
            for attempt in attempts:
                try:
                    with os.popen(
                        '%s "%s" %s/documents' % (attempt, TEMP_LOG_FILE, UPLOAD_LOC)
                    ) as f:
            ...
            if not key:
                ...
                if CALLER == "kodi":
                    ...
                    self.copy_to_boot = DIALOG.yesno(
                        lang(32013), lang(32023), lang(32039)
                    )
