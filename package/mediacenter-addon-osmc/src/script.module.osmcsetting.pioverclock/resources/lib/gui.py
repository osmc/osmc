...
class overclock_gui(xbmcgui.WindowXMLDialog):
    def __init__(self, strXMLname, strFallbackPath, strDefaultName, **kwargs):
        ...
        self.oc_profile = namedtuple(
            "Profile",
            "arm_freq sdram_freq core_freq initial_turbo over_voltage over_voltage_sdram force_turbo",
        )
        self.normal_profile = self.oc_profile(
            self.model["normal"]["arm_freq"],
        ...
        if self.model["medium"] is None:
            ...
            self.medium_profile = self.oc_profile(
                self.model["medium"]["arm_freq"],
        ...
        if self.model["higher"] is None:
            ...
            self.higher_profile = self.oc_profile(
                self.model["higher"]["arm_freq"],
        ...
        self.custom_profile = self.oc_profile(
            ...
            self.setting_values.get(
                "over_voltage_sdram", self.normal_profile.over_voltage_sdram
            ),
            self.setting_values.get("force_turbo", self.normal_profile.force_turbo),
        )

        self.overclock_profiles = [
            self.normal_profile,
            self.medium_profile,
            self.higher_profile,
            self.custom_profile,
        ...
        self.metrics = {
            self.control_matching["core_freq"]: self.oc_factor(
                150, 500, 10, self.custom_profile.core_freq
            ),
            self.control_matching["arm_freq"]: self.oc_factor(
                600, 600, 10, self.custom_profile.arm_freq
            ),
            self.control_matching["sdram_freq"]: self.oc_factor(
                300, 400, 10, self.custom_profile.sdram_freq
            ),
            self.control_matching["initial_turbo"]: self.oc_factor(
                0, 60, 5, self.custom_profile.initial_turbo
            ),
            self.control_matching["over_voltage"]: self.oc_factor(
                0, 8, 1, self.custom_profile.over_voltage
            ),
            self.control_matching["over_voltage_sdram"]: self.oc_factor(
                0, 8, 1, self.custom_profile.over_voltage_sdram
            ),
        }
    ...
    def check_for_profile(self, focus=False):
        ...
        for i, profile in enumerate(self.overclock_profiles):
            ...
            if i == 3:
                break  # i == 3 means the custom profile
    ...
    def set_description(self, focused_control):
        ...
        desc = self.descriptions.get(
            ...
            self.descriptions.get(
                focused_control - 1, self.descriptions.get(focused_control - 2)
            ),
        )
