...
def arm_freq_config_set(kodi_setting, all_settings):
    ...
    if int(kodi_setting) == PI_CPU_INFO["arm_freq"]:
        return "remove_this_line"
...
def sdram_freq_config_set(kodi_setting, all_settings):
    ...
    if int(kodi_setting) == PI_CPU_INFO["sdram_freq"]:
        return "remove_this_line"
...
def core_freq_config_set(kodi_setting, all_settings):
    ...
    if int(kodi_setting) == PI_CPU_INFO["core_freq"]:
        return "remove_this_line"
...
MASTER_SETTINGS = {
    "arm_freq": {
        ...
        "config_get_patterns": [
            {"identify": r"\s*arm_freq\s*=\s*", "extract": r"\s*arm_freq\s*=\s*(\d+)"}
        ],
    ...
    "core_freq": {
        ...
        "config_get_patterns": [
            {"identify": r"\s*core_freq\s*=\s*", "extract": r"\s*core_freq\s*=\s*(\d+)"}
        ],
