...
class OSMCGui(threading.Thread):
    ...
    def create_gui(self):
        ...
        self.order_of_fill = [
            item + (100 * x)
            for x in range(self.number_of_pages_needed)
            for item in self.item_order
        ]
        self.apply_buttons = [
            item + (100 * x)
            for x in range(self.number_of_pages_needed)
            for item in self.apply_button
        ]
        ...
        self.GUI = OSMC_gui(
            ...
            scriptPath,
            "Default",
            order_of_fill=self.order_of_fill,
    ...
    def load_widget_info(self):
        ...
        script_location = os.path.join(
            scriptPath, "resources", "lib", "call_osmc_parent.py"
        )
    ...
    def retrieve_modules(self):
        ...
        folders = [
            ...
            if os.path.isdir(os.path.join(addon_folder, item))
        ]

        osmc_modules = [
            x
            for x in [self.inspect_folder(addon_folder, folder) for folder in folders]
            if x
        ]
    ...
    def inspect_folder(self, addon_folder, sub_folder):
        ...
        if not os.path.isdir(osmc_subfolder):
            return
