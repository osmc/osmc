...
def toggle_technology_state(technology, state):
    ...
    technology_interface = dbus.Interface(
        bus.get_object(CONNMAN_OBJECT_PATH, "/net/connman/technology/" + technology),
...
def get_manager_interface():
    ...
    try:
        return dbus.Interface(
            bus.get_object(CONNMAN_OBJECT_PATH, "/"), "net.connman.Manager"
        )
...
def get_service_interface(path):
    ...
    try:
        return dbus.Interface(
            bus.get_object(CONNMAN_OBJECT_PATH, path), "net.connman.Service"
        )
    except dbus.DBusException as error:
