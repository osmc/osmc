...
def is_ethernet_enabled():
    return (
        connman.is_technology_enabled("ethernet")
        or not get_nfs_ip_cmdline_value() == None
    )
...
def apply_network_changes(settings_dict, internet_protocol):
    if settings_dict[internet_protocol]["Method"] in [
        "manual",
        "dhcp",
    ]:  # non NFS setup
        ...
        service.SetProperty(
            "Nameservers.Configuration", dbus.Array(dns, signature=dbus.Signature("s"))
        )
        service.SetProperty(
            "Nameservers.Configuration", dbus.Array(dns, signature=dbus.Signature("s"))
        )
        ...
        if settings_dict[internet_protocol]["Method"] == "nfs_manual":
            ip_value = create_cmdline_nfs_manual_string(
                settings_dict, internet_protocol
            )
...
def create_cmdline_nfs_manual_string(settings_dict, internet_protocol):
    cmd_string = (
        settings_dict[internet_protocol]["Address"]
        + "::"
        + settings_dict[internet_protocol]["Gateway"]
        + ":"
    ...
    for int in range(1, 3):
        count = str(int)
        if (
            "DNS_" + count in settings_dict["Nameservers"]
            and settings_dict["Nameservers"]["DNS_" + count]
        ):
...
def get_non_connman_connection_details():
    ...
    proc = subprocess.Popen(
        ["/sbin/ifconfig", device], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    (ifconfig_data, stderr) = proc.communicate()
    proc.wait()
    # parse ifconfig using re
    data = re.findall(
        r"^(\S+).*?inet addr:(\S+).*?Mask:(\S+)", ifconfig_data, re.S | re.M
    )
    ...
    # get default gateway
    proc = subprocess.Popen(
        ["/sbin/route", "-n"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
        ...
        for internet_protocol in ["IPV4", "IPV6"]:
            if internet_protocol in ethernet_settings:
                if "Method" in ethernet_settings[internet_protocol]:
                    if ethernet_settings[internet_protocol]["Method"].startswith(
                        "nfs_"
                    ):
                        if online:
