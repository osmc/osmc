...
class networking_gui(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        if self.use_preseed and not osmc_network.get_nfs_ip_cmdline_value():
            ...
            if self.preseed_data:
                if (
                    self.preseed_data["Interface"].startswith("wlan")
                    and osmc_network.is_wifi_available()
                ):
        ...
        for ctl in MAIN_MENU:
            self.getControl(ctl * 10).setVisible(
                True if ctl == panel_to_show else False
            )
    ...
    def setup_networking_from_preseed(self):
        ...
        if (
            self.preseed_data is None
            or self.preseed_data["Interface"].startswith("eth")
            or not connected
        ):
        ...
        if connected:
            ...
            if self.preseed_data is None or "dhcp" in self.preseed_data["Method"]:
                ...
                self.current_network_config["IPV4"]["Address"] = self.preseed_data[
                    "Address"
                ]
                self.current_network_config["IPV4"]["Netmask"] = self.preseed_data[
                    "Netmask"
                ]

                if self.preseed_data["Gateway"]:
                    self.current_network_config["IPV4"]["Gateway"] = self.preseed_data[
                        "Gateway"
                    ]

                if self.preseed_data["DNS_1"]:
                    self.current_network_config["IPV4"]["DNS_1"] = self.preseed_data[
                        "DNS_1"
                    ]

                if self.preseed_data["DNS_2"]:
                    self.current_network_config["IPV4"]["DNS_2"] = self.preseed_data[
                        "DNS_2"
                    ]
    ...
    def onClick(self, controlID):
        ...
        try:
            ...
            xbmc.executebuiltin(
                "XBMC.Notification(%s,%s,%s)" % ("Networking Add-on", message, "2500")
            )
    ...
    def onAction(self, action):
        ...
        if focused_control in MAIN_MENU:
            ...
            for ctl in MAIN_MENU:
                ...
                self.getControl(ctl * 10).setVisible(
                    True if ctl == focused_control else False
                )
    ...
    def edit_ip_address(self, controlID):
        ...
        current_label = relevant_label_control.getLabel().replace(
            "___ : ___ : ___ : ___", ""
        )

        user_input = DIALOG.input(
            lang(32004), current_label, type=xbmcgui.INPUT_IPADDRESS
        )
            ...
            if self.current_network_config:
                ...
                if self.current_network_config["State"] == "configuration":
                    ...
                    if osmc_network.has_network_connection(True):
                        # 'Status'                               'Connected'
                        status = (
                            lang(32044) + ": " + interface + " (" + lang(32046) + ")"
                        )
                    else:
                        # 'Status'                               'No internet'
                        status = (
                            lang(32044) + ": " + interface + " (" + lang(32047) + ")"
                        )
                ...
                if self.current_network_config["State"] in [
                    "online",
                    "ready",
                    "configuration",
                ]:
                    ...
                    self.update_manual_DHCP_button(
                        WIRED_DHCP_MANUAL_BUTTON, WIRED_IP_VALUES, WIRED_IP_LABELS
                    )
                    self.populate_ip_controls(
                        self.current_network_config, WIRED_IP_VALUES
                    )
    ...
    def populate_mysql(self, dictionary):
        ...
        if music:
            ...
            name, host, port, user, pswd, hpwd = (
                self.getControl(x) for x in MYSQL_MUSIC_VALUES
            )
    ...
    def read_mysql_settings(self, dictionary):
        ...
        if self.getControl(MYSQL_VIDEO_TOGGLE).isSelected():
            ...
            defaults = {"name": "MyVideos", "port": "3306"}
            for sql_item, ctl in zip(sql_subitems, MYSQL_VIDEO_VALUES):
                ...
                log(
                    "ctl %s : sql item %s : %s"
                    % (ctl, sql_item, self.getControl(ctl).getLabel())
                )
        ...
        if self.getControl(MYSQL_MUSIC_TOGGLE).isSelected():
            ...
            defaults = {"name": "MyMusic", "port": "3306"}
    ...
    def write_advancedsettings(self, dictionary):
        ...
        dictionary_valid, invalidity_type = self.ASE.validate_advset_dict(
            dictionary, no_pw_ok=True
        )

        if not dictionary_valid:
            if invalidity_type == "missing mysql":

                discard = DIALOG.yesno(
                    "OSMC",
                    "MySQL settings are incomplete.",
                    "Discard MySQL settings changes?",
                )
        ...
        try:
            ...
            if resp:
                ...
                try:
                    ...
                    log(
                        "Attempt to change permission on advancedsettings.xml file has failed."
                    )
                    ok = DIALOG.ok(
                        "OSMC",
                        "Failed to change permission restrictions.",
                ...
                if res == 0:
                    log(
                        "Second attempt at writing MySQL changes to advancedsettings.xml file."
                    )
    ...
    def user_entry_mysql(self, controlID):
        ...
        if controlID in MYSQL_USER:
            ...
            new_val = DIALOG.input(
                "Please Enter MySQL Username", current, type=xbmcgui.INPUT_ALPHANUM
            )
            ...
            new_val = DIALOG.input(
                "Please Enter MySQL Port Number", current, type=xbmcgui.INPUT_NUMERIC
            )

        elif controlID in MYSQL_NAME:

            new_val = DIALOG.input(
                "Please Enter New Name", current, type=xbmcgui.INPUT_ALPHANUM
            )
    ...
    def populate_ip_controls(self, settings_dict, controls):
        ...
        ip_address, subnet, defaultGateway, primaryDNS, secondaryDNS = (
    ...
    def handle_wired_selection(self, control_id):
        ...
        if control_id == WIRED_DHCP_MANUAL_BUTTON:
            ...
            if self.current_network_config[self.internet_protocol]["Method"] == "dhcp":
                ...
                self.current_network_config[self.internet_protocol]["Method"]
                == "nfs_dhcp"
            ):
                self.current_network_config[self.internet_protocol][
                    "Method"
        ...
        if control_id == WIRED_APPLY_BUTTON:
            if self.current_network_config:
                osmc_network.apply_network_changes(
                    self.current_network_config, self.internet_protocol
                )
    ...
    def toggle_ethernet(self):
        ...
        if ethernet_state:
            ...
            while changed_state != 0 and changed_state < 20:
                ...
                if (
                    settings
                    and "State" in list(settings.keys())
                    and settings["State"] in ("ready", "online")
                ):
                    changed_state = 0
                else:
                    changed_state += 1
    ...
    def update_apply_reset_button(self, net_type):
        ...
        if net_type == "WIRELESS":
            ...
            if cmp(wireless_config, self.current_network_config) == 0:
                ...
                self.toggle_controls(
                    True, [WIRELESS_RESET_BUTTON, WIRELESS_APPLY_BUTTON]
                )
            ...
            if osmc_network.is_wifi_enabled():
                ...
                if self.conn_ssid:
                    ...
                    if "Address" in self.current_network_config[self.internet_protocol]:
                        ...
                        self.populate_ip_controls(
                            self.current_network_config, WIRELESS_IP_VALUES
                        )

                    self.toggle_controls(
                        True,
                        [
                            WIRELESS_ADAPTER_TOGGLE,
                            WIRELESS_NETWORKS,
                            WIRELESS_DHCP_MANUAL_BUTTON,
                        ],
                    )
    ...
    def handle_wireless_selection(self, control_id):
        ...
        if control_id == 5000:  # wireless network
            ...
            self.update_manual_DHCP_button(
                WIRELESS_DHCP_MANUAL_BUTTON, WIRELESS_IP_VALUES, WIRELESS_IP_LABELS
            )
    ...
    def handle_selected_wireless_network(self):
        ...
        if connected == "False":
            if not self.conn_ssid:  # if we are not connected to a network connect
                ...
                # 'Wireless'
                xbmc.executebuiltin(
                    "XBMC.Notification(%s,%s,%s)" % (lang(32041), message, "2500")
                )
        ...
        if wifi:
            ...
            if connect or hiddenssid:
                ...
                connection_status = osmc_network.wifi_connect(
                    path, None, hiddenssid, script_base_path
                )
                ...
                if not connection_status and encrypted:
                    ...
                    if password:
                        ...
                        connection_status = osmc_network.wifi_connect(
                            path, password, hiddenssid, script_base_path
                        )
                        self.clear_busy_dialogue()
    ...
    def handle_bluetooth_selection(self, control_id):
        ...
        if control_id == BLUETOOTH_ENABLE_TOGGLE:  # Enable Bluetooth
            ...
            if item:
                ...
                selection = DIALOG.select(
                    alias, [lang(32051), lang(32075), lang(32021)]
                )
            ...
            if item:
                ...
                selection = DIALOG.select(
                    lang(32022) + " " + alias + "?",
                    [lang(32055), lang(32056), lang(32057)],
                )
    ...
    def populate_tethering_panel(self):
        ...
        control_label = "".join(
            [x if i % 33 else " " + x for i, x in enumerate(self.hotspot_passphrase)]
        )
    ...
    def handle_tethering_selection(self, control_id):
        ...
        if control_id == TETHERING_WIFI_PASSPHRASE_LABEL:
            ...
            if enteredPassphrase:
                ...
                control_label = "".join(
                    [x if i % 33 else " " + x for i, x in enumerate(enteredPassphrase)]
                )
...
class bluetooth_population_thread(threading.Thread):
    ...
    def update_bluetooth_lists(self):
        ...
        self.update_list_control(
            dict(self.discovered_dict), self.discovered_list_control
        )
    ...
    def update_list_control(self, devices_dict, list_control):
        ...
        list(
            map(
                lambda address_info: list_control.addItem(
                    self.create_bluetooth_item(address_info[0], address_info[1])
                ),
                iter(devices_dict.items()),
            )
        )
    ...
    def populate_bluetooth_dict(self, paired):
        ...
        try:
            devices = (
                osmc_bluetooth.list_trusted_devices()
                if paired
...
class wifi_populate_bot(threading.Thread):
    ...
    def update_list_control(self, running_dict, multiAdpter):
        ...
        for itemIndex in range(0, self.WFP.size()):
            ...
            if listItemAdapterAddress in list(running_dict.keys()):
                if listItemSSID in list(running_dict[listItemAdapterAddress].keys()):
                    ...
                    if (
                        listItem.getLabel() == expectedLabel
                        and listItemConnected == networkConnected
                    ):
        ...
        if prevSelectedItem is not None:
            ...
            for itemIndex in range(0, self.WFP.size()):
                ...
                if (
                    listItemAdapterAddress == selectedAdapterAddress
                    and listItemSSID == selectedSSID
                ):
    ...
    def get_wifi_icon(encrypted, strength, connected):
        ...
        if strength >= 0 and strength <= 40:
            ...
            filename += "6080"
        elif strength >= 61 and strength <= 100:
            filename += "80100"
