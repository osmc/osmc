...
def list_devices(filterkey=None, expectedvalue=None):
    ...
    for path in list(managed_objects.keys()):
        if path.startswith("/org/bluez/hci") and DEVICE_PATH in list(
            managed_objects[path].keys()
        ):
    ...
    while True:
        try:
            ...
            if RUNNING_IN_KODI:
                ...
                returnValue = handleAgentInteraction(
                    deviceAlias, return_value, messages
                )
                if returnValue:
...
def handleAgentInteraction(deviceAlias, command, messages):
    ...
    if messages[0] == "AUTHORIZE_SERVICE":
        ...
        message = (
            lang(32027) + " " + messages[0] + " " + lang(32029) + " " + deviceAlias
        )
    ...
    if messages[0] == "CONFIRM_PASSKEY":
        #           'Confirm passkey'                      'for'
        message = (
            lang(32031) + " " + str(messages[1]) + " " + lang(32032) + " " + deviceAlias
        )
