...
def sanitise_file(file_loc):
    ...
    with open(file_loc, 'r') as f:
        ...
        for line in lines:
            ...
            # if the line is not a new dtparam, then role those lines up together and appened to newlines
            ...
            if 'dtoverlay' in line:
                overlay_count += 1
                overlay_storage.append(line.strip().replace('dtoverlay', 'dtoverlay_COUNT%s' % overlay_count))
...
file_loc = 'C:\\temp\\config.txt'
expo_loc = 'C:\\temp\\temp.txt'



sanitised_file = sanitise_file(file_loc)

# sanitised_file.seek(0)
# for line in sanitised_file.readlines():
#     print line


# sys.exit()

# with open(expo_loc, 'w') as f:
...
writeIO = StringIO.StringIO()

config = ConfigObj(infile=sanitised_file, write_empty_values=True)

config['arm_freq_min'] = 1

print config['arm_freq_min']

config.write(writeIO)

def write_to_config_file(export_location, config_stringIO):
    ...
    with open(export_location, 'w') as f:
        ...
        for line in config_stringIO.readlines():
            ...
            if '_COUNT' in line and ' =' in line:
                x = line.index('_COUNT')
                y = line.index(' =')
                line = line[:x] + line[y:]

            line = line.replace(' = ', '=').replace('_SPACE_', ' ').replace('=NO_EQUALS_SIGN','').replace(', ',',')
            f.write(line)


write_to_config_file(expo_loc, writeIO)
