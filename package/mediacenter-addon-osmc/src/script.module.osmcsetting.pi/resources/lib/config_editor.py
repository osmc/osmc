...
class ConfigEditor(xbmcgui.WindowXMLDialog):
    def onInit(self):
        ...
        dup_check = [
            y
            for y in [x.split("=")[0] for x in self.grab_item_strings() if "=" in x]
    ...
    def onClick(self, controlID):
        ...
        if controlID == SAVE:
            ...
            if self.changed:
                ...
                final_action = DIALOG.yesno(
                    lang(32052), lang(32053), nolabel=lang(32054), yeslabel=lang(32055)
                )
            ...
            if selected_entry != 0:
                ...
                if self.del_string not in currentlabel:
                    ...
                    if action:
                        ...
                        # edit
                        d = DIALOG.input(
                            lang(32060), currentlabel, type=xbmcgui.INPUT_ALPHANUM
                        )
                    ...
                    if action:
                        # delete
                        item.setLabel(
                            currentlabel[: len(currentlabel) - len(self.del_string)]
                        )
                        self.changed = True
                        ...
                        # edit
                        d = DIALOG.input(
                            lang(32063), currentlabel, type=xbmcgui.INPUT_ALPHANUM
                        )
    ...
    def check_for_duplicates(self, d, edit=False):
        ...
        if "=" in d:
            ...
            if dupe not in self.ignore_list and (
                (edit and dupe_check.count(dupe) > 1)
                or (not edit and dupe in dupe_check)
            ):
