...
Overclock settings are set using the Pi Overclock module."""
    ...
    def run(self):
        ...
        new_settings = parser.kodi_to_config(
            parser.MASTER_SETTINGS, config, new_settings
        )
        print("K" * 50)
        print(new_settings)
        print("K" * 50)
    ...
    def settings_retriever_xml(self):
        ...
        print("*" * 100)
        print("Settings gotten from kodi")
        for key in list(parser.MASTER_SETTINGS.keys()):
            print("%s: %s" % (key, addon.getSetting(key)))
    ...
    def populate_misc_info(self):
        ...
        serial = (
            serial_raw[serial_raw.index("Serial") + len("Serial") :]
            .replace("\n", "")
            .replace(":", "")
            .replace(" ", "")
            .replace("\t", "")
