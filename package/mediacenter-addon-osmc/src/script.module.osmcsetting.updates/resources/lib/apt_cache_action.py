...
class Main(object):
    ...
    def respond(self):
        ...
        if self.action == "action_list" and self.error_message != "":
            ...
            call_parent(
                "apt_action_list_error",
                {"error": self.error_message, "package": self.error_package},
            )
    ...
    def update(self):
        ...
        call_parent(
            "progress_bar",
            {"percent": 1, "heading": self.heading, "message": "Cache Updating"},
        )
    ...
    def commit_action(self):
        ...
        call_parent(
            "progress_bar",
            {"percent": 100, "heading": self.heading, "message": "Commit Complete"},
        )
    ...
    def fetch(self):
        ...
        call_parent(
            "progress_bar",
            {"percent": 100, "heading": self.heading, "message": "Downloads Complete"},
        )
...
class Operation_Progress(apt.progress.base.OpProgress):
    ...
    def update(self):
        ...
        call_parent(
            "progress_bar",
            {"percent": self.percent, "heading": self.op, "message": self.sub_op},
        )
...
class Download_Progress(apt.progress.base.AcquireProgress):
    def __init__(self, partial_heading="Downloading"):
        ...
        call_parent(
            "progress_bar",
            {"percent": 0, "heading": "Downloading Update", "message": self.fetching},
        )
    ...
    def pulse(self, owner=None):
        ...
        if (diff.total_seconds() * 10) < 11:
            ...
            hdg = "{0:d} / {1:d} items  --  {2:}  --  {3:.1f} / {4:.1f}MB".format(
                self.current_items + 1, self.total_items, cps, cmb, tmb
            )

            call_parent(
                "progress_bar", {"percent": pct, "heading": hdg, "message": msg}
            )
