...
class SimpleScheduler(object):
    ...
    def set_trigger(self, right_now):
        ...
        if self.frequency == 1:
            ...
            # if the user wants a specific time, then use that, otherwise use a random time
            self.trigger_time = self.set_trigger_time(
                right_now + datetime.timedelta(days=delta_days)
            )
    ...
    def set_trigger_time(self, trigger_time):
        ...
        if self.specific_time:
            ...
            new_trigger = trigger_time.replace(
    ...
    def step_trigger(self):
        ...
        if self.frequency == 1:
            ...
            if self.daynum > 0:
                ...
                year = (
                    self.trigger_time.year + 1 if month == 1 else self.trigger_time.year
                )

                self.trigger_time = self.trigger_time.replace(
                    year=year, month=month, day=self.daynum
                )
