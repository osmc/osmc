...
settings = {"ip": "127.0.0.1", "port": "80", "user": "", "pssw": ""}

keymap = {"i": "ip", "p": "port", "u": "user", "w": "pssw"}
...
try:
    ...
    print(
        "All the settings are optional. The default will be used in their place if you dont specifiy them."
    )
    print("Defaults:")
    print("		ip   : 127.0.0.1")
    print("		port : 80")
    ...
    print("")
    print(
        "Alternatively, you can save a file called /home/osmc/cli_remote.conf with this:"
    )
    print(
...
def call(settings, action, params=None):
    ...
    r = requests.post(
        url, data=data, headers=headers, auth=(settings["user"], settings["pssw"])
    )
    ...
    data = json.dumps(
        {
            ...
            "params": {"title": "Kodi CLI Remote", "message": "Connected!"},
            "id": 1,
        }
    )
    r = requests.post(
...
def redraw(stdscr):
    ...
    stdscr.addstr(
        11, 0, ">>> 'k' to enter keyboard mode (send text to Kodi's keyboard)"
    )
