...
pull_source "$REPO/archive/${REVISION}.tar.gz" "$(pwd)/kodi"
fi
else
...
then
	...
	dpkg_build files/ ${1}-mediacenter-next-osmc.deb
	build_return=$?
fi
teardown_env "${1}"
exit $build_return
