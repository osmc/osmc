...
{
	if ! whereis unzip >/dev/null 2>&1; then echo -e "Installing unzip" && update_sources && verify_action && install_package "unzip" && verify_action; fi
	if ! whereis git >/dev/null 2>&1; then echo -e "Installing Git" && update_sources && verify_action && install_package "git" && verify_action; fi
	if ! whereis svn >/dev/null 2>&1; then echo -e "Installing Subversion" && update_sources && verify_action && install_package "subversion" && verify_action; fi
	if [ -d ${2} ]
	then 
		if [ "$2" != "." ]
		then
