...
. ../common/funcs.sh
wd=$(pwd)
filestub="osmc-vero5-filesystem"
...
# and any install or dist-upgrade touching one fails outright. OSMC mirrors the
# final published state of the suite, so point at that instead of security.debian.org.
...
chroot ${DIR} apt-get -y install --no-install-recommends vero5-userland-osmc
verify_action
chroot ${DIR} apt-get -y install --no-install-recommends vero5-device-osmc
verify_action
